...
- **长会话被自动压缩后，过程性回复改用英文**（用户实测反馈）。核对结论：额外上下文本身既没丢、也仍在生效（压缩后每个回合的最终答复依旧以 `✅` 结尾，而 `✅` 只可能来自 system prompt 的额外上下文）；真正的原因是 DSH 生成压缩摘要的那次调用自带一条固定英文写作要求（`dsh-compaction-basic` 的 `Write concise English engineering prose`），摘要因此是英文，而摘要替换掉整段会话后成了系统提示词之后最近的上下文。
- 新增：对 `purpose === 'compaction'` 的模型请求，把用户额外上下文原文追加到该请求末尾（排在 DSH 那条英文指令之后），并要求摘要沿用额外上下文规定的输出语言、同时不要把给用户回复用的装饰（如结尾标记）搬进摘要。该消息只存在于这一次请求里，不写入会话记录；请求形状或 `options` 异常时一律放行原请求，不影响压缩本身。
