...
VERIFIED_DSH_VERSIONS=("0.1.6-alpha.1")
...
if ! is_verified_dsh_version "$NORMALIZED_DSH_VERSION"; then
  printf '警告: DSH %s 位于兼容发布线内，但尚未列入逐版本验证清单（%s）；运行时能力检查继续 fail closed。\n' "$ACTUAL_DSH_VERSION" "${VERIFIED_DSH_VERSIONS[*]}" >&2
fi
...
安装完成。需要注意:
  ...
  2. 重启后打开 设置 → 额外上下文 编辑规则；文本写入 ~/.dsh/settings.yaml 的 extra-context 段。
EOF
