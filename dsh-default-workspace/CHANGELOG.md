...
- **兼容线移到 `>=0.1.7-alpha.1 <0.1.8`**（`0.1.7-alpha.1` 已逐版本核对）：`package.json`、`engines.dsh`、`install.sh` 版本门与 `lib/index.js` 四处同源更新，范围外保持 inert。
- **图标名适配 0.1.7 的改名**（数字档位 → 字重档位）：按能力取用（`iconOf()` 候选序列），壳层自己的「新建会话」按钮在宽栏用 `IconNewChatOutlineMedium`、窄栏用 `IconNewChatOutlineRegular`，本插件跟随同一档位。
- **与核心新增的「默认工作区」划清分工**：0.1.7 核心在「工作区列表与会话历史都为空」时会自动建一个标题「默认工作区」的工作区（无重命名/删除保护、无独立入口）。插件仍负责受管的「通用会话」（固定目录、置顶、方法级保护、独立新建入口），并在 `/dsh-default-workspace/status` 的 `?debug=1` 里报告核心是否具备该能力（`coreDefaultWorkspace`），便于排查"两个默认工作区"。
