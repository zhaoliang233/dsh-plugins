...
固定场景里的 React 垫片与 ctx 桩按"够跑真实插件"维护，遇到新插件用到的 API 就补上，别让插件为工具让路：

- `createContext` / `useContext` 与 `Provider`（把 ctx/scope 往下传是常见写法，缺了它 bundle 在 factory 阶段就抛错）；
- `ctx.effect(fn)` 会**立即**执行 `fn` 并登记返回的清理函数——少了它，把副作用写在 effect 回调里的插件在场景里什么都不做，
  量出来的失败是场景的错、不是插件的错。

`--label` 必须是该分区在导航里的**精确菜单名**：补丁按名字精确匹配，传错名字会得到"补丁没生效"的假失败
（`dsh-chat-archive-manager` 的菜单名是「归档管理」，曾用「归档会话」跑出过三条假失败）。
